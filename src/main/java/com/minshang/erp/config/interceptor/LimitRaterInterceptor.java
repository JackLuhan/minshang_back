package com.minshang.erp.config.interceptor;
...
public class LimitRaterInterceptor extends HandlerInterceptorAdapter {
    ...
    @Value("${minshang.rateLimit.enable}")
    private boolean rateLimitEnable;

    @Value("${minshang.rateLimit.limit}")
    private Integer limit;

    @Value("${minshang.rateLimit.timeout}")
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if (rateLimiter != null) {
            ...
            if (StrUtil.isBlank(token3)) {
                throw new MinShangException("当前访问人数太多啦，请稍后再试");
            }
        }
        return true;
    }
